...
// 6、圣杯模式的继承
export function inherit(Target, Origin) {
    function F() {}
...
// 9、 返回元素的第n个兄弟节点
export function retSibling(e, n) {
    while (e && n) {
        if (n > 0) {
...
// 23、运动函数
export function animate(obj, json, callback) {
...
// 24、弹性运动
export function ElasticMovement(obj, target) {
    clearInterval(obj.timer);
    var iSpeed = 40,
        a,
        u = 0.8;
...
// 33、原生js封装ajax
export function ajax(method, url, callback, data, flag) {
    var xhr;
    flag = flag || true;
...
// 49、单例模式
export function getSingle(func) {
    var result;
    return function () {
        if (!result) {
