...
export const date = {
    ...
    time: (t = new Date()) => (is.date(t) ? t : new Date(t)).getTime(),
    getNextDate(nDays = 1, fmt = "") {
        const time = new Date(date.time() + 24 * 60 * 60 * 1000 * Number(nDays));
    ...
    getTimeBucket(dayArr = [7, 1 * 30, 3 * 30], fmt = "") {
        ...
        return dayArr.map((v, i) => ({
            begin: fmt ? date.formatTime(t - Number(v) * h24, fmt) : t - Number(v) * h24,
